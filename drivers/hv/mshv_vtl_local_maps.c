...
#define MAX_HYPERV_CPUS 			(u64)(2048)
...
static void mshv_vtl_teardown_local_maps_no_lock(struct mshv_local_maps *maps)
{
	...
	list_for_each_entry_safe(maps_entry, tmp, &maps->node, node) {
		pr_debug("freeing page table %#llx\n", (u64)(maps_entry->table));
...
static int mshv_vtl_local_maps_open(struct inode *node, struct file *f)
{
	pid_t pid = task_pid_vnr(current);
	uid_t uid = current_uid().val;

	pr_info("%s: Creating local maps, task group %d, uid %d\n", __func__, pid, uid);

	if (!capable(CAP_SYS_ADMIN))
		return -EPERM;
	f->private_data = mshv_vtl_setup_local_maps();
	if (!f->private_data)
		return -ENOMEM;
...
static int mshv_vtl_local_maps_release(struct inode *node, struct file *f)
{
	pid_t pid = task_pid_vnr(current);
	uid_t uid = current_uid().val;

	pr_info("%s: Tearing local maps down, task group %d, uid %d\n", __func__, pid, uid);

	if (f->private_data)
		mshv_vtl_teardown_local_maps((struct mshv_local_maps *)f->private_data);
	f->private_data = NULL;

	return 0;
}

static const struct file_operations mshv_vtl_local_maps_file_ops = {
	.owner = THIS_MODULE,
	.open = mshv_vtl_local_maps_open,
	.release = mshv_vtl_local_maps_release,
};

static struct miscdevice mshv_vtl_local_maps = {
	.name = "mshv_local_maps",
	.nodename = "mshv_local_maps",
	.fops = &mshv_vtl_local_maps_file_ops,
	.mode = 0600,
	.minor = MISC_DYNAMIC_MINOR,
};


static int __init mshv_vtl_local_maps_init(void)
{
	return misc_register(&mshv_vtl_local_maps);
}

static void __exit mshv_vtl_local_maps_exit(void)
{
	misc_deregister(&mshv_vtl_local_maps);
}

module_init(mshv_vtl_local_maps_init);
module_exit(mshv_vtl_local_maps_exit);
